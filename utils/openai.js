...
async function generateScript(conversations) {
  const completion = await openai.chat.completions.create({
    messages: [
      ...
      {
        role: 'assistant',
        content: '¿Cuál debería de ser la estructura de tu guion?'
      },
      {
        role: 'user',
